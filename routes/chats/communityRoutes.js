...
router.get('/groups/by-interest/:tag', authenticateUser, communityController.getGroupsByTag);

router.get('/groups/search', authenticateUser, communityController.searchGroups);

// ✅ Get a specific group's info by ID
router.get('/groups/:groupId', authenticateUser, communityController.getGroupById);

// ✅ Join a group (user must be logged in)
router.post('/join/:groupId', authenticateUser, communityController.joinGroup);

// ✅ Leave a group
router.delete('/leave/:groupId', authenticateUser, communityController.leaveGroup);
...
router.get('/:groupId/messages', authenticateUser, communityController.getGroupMessages);
